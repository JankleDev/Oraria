...
type AttributeKey = "vitality" | "endurance" | "strength" | "dexterity" | "intelligence" | "faith" | "arcane";
type ResourceKey = "health" | "stamina" | "mana" | "focus";
type ScalingCurve = "standard" | "strong" | "weak" | "magic";
...
export class Attributes {
	...
	static readonly SOFT_CAPS: Record<AttributeKey, SoftCap> = {
	...
	private _currentResources: ResourceValues;
	private _maxResources: ResourceValues;
	...
	get baseAttributes(): Readonly<BaseAttributes> {
		return { ...this._baseAttributes };
	}

	get effectiveAttributes(): Record<AttributeKey, number> {
		return this.calculateEffectiveAttributes();
	}

	increaseAttribute(attribute: AttributeKey, amount: number = 1): boolean {
	...
	private calculateEffectiveAttributes(): Record<AttributeKey, number> {
		const effective: Partial<Record<AttributeKey, number>> = {};

		(Object.keys(this._baseAttributes) as AttributeKey[]).forEach((attr) => {
		...
		return effective as Record<AttributeKey, number>;
	}
	...
	reduceHealth(amount: number): number {
		this._currentResources.health = Math.max(0, this.health - amount);
		return this.health;
	}

	reduceStamina(amount: number): number {
	...
	reduceFocus(amount: number): number {
	...
	consumeResource(resourceType: symbol, amount: number): number {
		switch (resourceType) {
			case Attributes.ResourceType.HEALTH:
	...
	// --- Defense and Scaling ---
	getDefenseRating(damageType: symbol): number {
	...
	getScalingFactor(attribute: AttributeKey, scalingCurve: ScalingCurve = "standard"): number {
